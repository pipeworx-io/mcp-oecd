...
`tools/list` at `https://gateway.pipeworx.io/oecd/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/oecd_list_curated_flows \
  -H 'Content-Type: application/json' \
  -d '{"topic":"gdp"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/oecd_list_curated_flows`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.

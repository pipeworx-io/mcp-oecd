...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'OECD');
}
  ...
  switch (name) {
    ...
    case 'search_dataflows':
      return searchDataflows(
        reqStr(args, 'query', '"household debt" or "education"'),
...
async function searchDataflows(query: string, agency: string | undefined, limit: number) {
  ...
  const res = await pwFetch(url, { headers: { Accept: 'application/vnd.sdmx.structure+json;version=1.0' } });
  if (!res.ok) {
    throw await httpError(res, 'OECD dataflow registry error');
  }
